...
/// сохраняем итемы...
bool uoHeaderScale::onStoreItems(uoReportLoader* loader, QList<int>* list, bool erase ){
...
void uoHeaderScale::onDeleteItem(uoRptNumLine* delItem){Q_UNUSED(delItem); }
...
void uoReportDocFontColl::copyFrom( uoReportDocFontColl *col){
	clear();
	for (int i = 0; i< col->countFonts(); i++)	{
		QFont* fontS = col->getFont(i);
		if (fontS)
			addFont(fontS->family());
	}
...
void uoCell::clear()
{
	if(m_textProp)		delete m_textProp; m_textProp = 0;
	if(m_borderProp)	delete m_borderProp; m_borderProp = 0;
	if(m_ceelJoin)		delete m_ceelJoin; m_ceelJoin = 0;
	uoTextBoundary* thisTB = m_textBoundary; m_textBoundary = 0;
	uoTextBoundary* thisAdd = 0;
	while(thisTB){
		thisAdd = thisTB->m_textBoundary;
		delete thisTB;
		thisTB = thisAdd;
	}
	m_props = 0;
...
QString uoCell::getTextWithLineBreak(bool drawInv)
{
	...
	if (m_textProp){
		...
		if (drawInv) {
			...
			if (m_textProp->m_alignFlags & uoCP_ContainsTab)
				cellTxt = cellTxt.replace('\t' ,str2192);

		} else {
			if (m_textProp->m_alignFlags & uoCP_ContainsLineBreak)
				cellTxt = cellTxt.replace('\n' ,str3);
...
void uoCell::copyFrom(uoCell* fromSrc, uoReportDoc* doc, uoReportDoc* docSrc, int rowNo)
{
	...
	if (!m_ceelJoin && fromSrc->m_ceelJoin) {
		m_ceelJoin = doc->getCellJoinStruct();
	}
	if (m_ceelJoin)
		m_ceelJoin->copyFrom(fromSrc->m_ceelJoin);
	m_props = fromSrc->m_props;
...
bool uoCell::alignmentForSimplDraw()
{
	bool retVl = false;
	if (m_textProp){
		if ((m_textProp->m_vertTAlignment == uoVA_Top) &&
		(m_textProp->m_horTAlignment == uoHA_Left) &&
...
void uoCell::setTextType(uoCellTextType tt, uoReportDoc *doc){
	if (!m_textProp)
		m_textProp = doc->getDefaultTextProp();
	if (m_textProp)
		m_textProp->m_textType = tt;
}
...
#ifdef Q_OS_WIN
unsigned int uoCell::getWinAligment() const
{
	...
	if (m_textProp){
		...
		{
			case uoHA_Unknown:
			case uoHA_Left:		{break;}
			case uoHA_Center:	{flags |= DT_CENTER; break;}
			case uoHA_Right:	{flags |= DT_RIGHT; break;}
			default:			{break;			}
		}
		switch(m_textProp->m_vertTAlignment)
		{
			case uoVA_Unknown:
			case uoVA_Top:		{break;}
			case uoVA_Center:	{flags |= DT_VCENTER ; break;}
			case uoVA_Bottom:	{flags |= DT_BOTTOM ; break;}
			default:			{break;			}
		}
		if (m_textProp->m_TextBehavior == uoCTB_Transfer)
			flags |= DT_WORDBREAK;
...
/// Или не входящая в объединение или первая
bool uoCell::isIndependentCell() const{
	if (m_ceelJoin){
		if (isFirstOfUnionCell())
			return true;
		else
			return false;
	}
...
void uoCell::setText(const QString& text)
{
	...
	if (m_textProp){
		m_textProp->m_alignFlags = 0;
		if (text.contains('\t'))
			m_textProp->m_alignFlags |= uoCP_ContainsTab;
		if (text.contains('\n'))
			m_textProp->m_alignFlags |= uoCP_ContainsLineBreak;
	}
}
...
uorPoint drawBorderPoint1, drawBorderPoint2; // Попробую исключить ненужные конструкторы/деструкторы
...
void uoCell::drawBorder(uoPainter& painter, uorRect& rectCell) const
{
	...
	for (int i=0; i<4; i++)	{
		...
		switch (i){
			case 0: // левый
			{
				drawBorderPoint1 = rectCell.bottomLeft();
				drawBorderPoint2 = rectCell.topLeft();
				break;
			}
			case 1: // верхний
			{
				drawBorderPoint1 = rectCell.topLeft();
				drawBorderPoint2 = rectCell.topRight();
//				painter.drawLine(drawBorderPoint1, drawBorderPoint2);
				break;
			}
			case 2: // правый
			{
				drawBorderPoint1 = rectCell.topRight();
				drawBorderPoint2 = rectCell.bottomRight();
//				painter.drawLine(drawBorderPoint1, drawBorderPoint2);
				break;
			}
			case 3: // низ
			{
				drawBorderPoint1 = rectCell.bottomRight();
				drawBorderPoint2 = rectCell.bottomLeft();
//				painter.drawLine(drawBorderPoint1, drawBorderPoint2);
				break;
			}
			default:
		...
		if (bt != uoCBT_CustomDashLine){
			...
			drawBorder(painter, drawBorderPoint1, drawBorderPoint2);
		}
	}
}
...
void uoRow::setNumber(const int &nm)
{
	m_number = nm;
	uoCell* cell = 0;
	for (int i = getMinNo(); i<=getMaxNo(); i++){
		cell = getItem(i, false);
		if (cell){
			cell->m_rowNo = nm;
		}
	}
...
void uoRow::copyFromTempl(uoRow *fromSrc, uoReportDoc *docThere, uoReportDoc *docSrc, int startColNo, int endColNo)
{
	if (fromSrc->isEmpty())
		return;
	int startNoSrc = fromSrc->getMinNo(), endNoSrc = fromSrc->getMaxNo();
	...
	uorNumber sSize = docSrc->getScaleSize(uorRhtRowsHeader, fromSrc->number());
	docThere->setScaleSize(uorRhtRowsHeader, number(), sSize);
	bool sHide = docSrc->getScaleHide(uorRhtRowsHeader, fromSrc->number());
	docThere->setScalesHide(uorRhtRowsHeader, number(), 1,sHide);
	bool sFixed = docSrc->getScaleFixedProp(uorRhtRowsHeader, fromSrc->number());
	docThere->setScaleFixedProp(uorRhtRowsHeader, number(), sFixed);
	...
	for (int i = startNoSrc; i <= endNoSrc; i++){
		...
		cellFrom = fromSrc->getItem(i, false);
		if (cellFrom){
			cellThere = docThere->getCell(number() , i , true, true);
			cellThere->copyFrom(cellFrom, docThere, docSrc, number());
		}
	}
}
...
void uoRow::saveItems(uoReportLoader* loader, uoReportSelection* selection, bool erase){
	detachIter();
	if (m_map->isEmpty())
		return;
...
void uoRowsDoc::saveItems(uoReportLoader* loader, uoReportSelection* selection, bool erase)
{
	...
	while(itLst != m_map->end()) {
		...
		if (item) {
			...
			if (saveThisRow) {
				...
				item->saveItems(loader, selection,erase);
				if (loader)
