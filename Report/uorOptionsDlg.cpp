...
void uorOptionsDlg::setUsePixCacher(bool dd)
{
	m_usePixCacher->setCheckState( dd ? Qt::Checked : Qt::Unchecked);

}
bool uorOptionsDlg::usePixCacher(){
	Qt::CheckState state = m_usePixCacher->checkState();
	return (state == Qt::Checked) ? true : false;
}
