...
{
	uorSelVisitorBase()
		:m_ctrl(0),m_doc(0),m_type(uorSVT_Unknown),m_needCreate(false)
	{
...
class uoReportCtrl : public QWidget
{
    ...
    friend struct uorReportAreaBase;
    ...
    protected:
		...
		bool findScaleLocation(uorNumber posX, uorNumber posY, int &scaleNo, uoRptHeaderType rht, bool saveStartRc = false);
		...
		uorRect			m_curMouseSparesRectStart;
		/**
    ...
    protected:
		...
		*/
		...
		long m_forseRedraw; 	 // перерисовка сейчас идет только при изменении некоторых опций в checkRedrawOption(), этот флаг - принудиловка.
...
//		void onRowColGroupOperation2(const uorRCGroupOperationType& opertn, const QList<int>& psList, const uoRptHeaderType& psRht) const;
	...
	private:
        ...
        QSize  m_lastSize;          ///<Размер окна редактора.
		...
		uoRptGroupItemList* m_groupListCols;		///< список ректов группировок столбцов
		uoRptGroupItemList* m_groupListRows;		///< список ректов группировок строк

		uoRptSectionItemList* m_sectItemListCache;	///< кешь для экземпляров uoRptSectionItem
		uoRptSectionItemList* m_sectItemListCols;		///< список итемов секций столбцов
		uoRptSectionItemList* m_sectItemListRows;		///< список итемов секций строк
