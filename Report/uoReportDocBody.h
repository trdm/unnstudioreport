...
struct uoRptNumLine : public uoEnumeratedItem{
	public:
		...
		uoRptNumLine(int ln) : m_lineNo(ln), m_size(-1), m_hiden(false), m_fixed(false),m_textDecor(0)	{}
		virtual ~uoRptNumLine(){}

	void 	setNumber(const int& nm) {m_lineNo = nm;}
	int  	number() {return m_lineNo;}

	void 	setSize(const uorNumber& sz, bool isDef = false) {
		if (isDef)	m_sizeDef = sz;
		else 		m_size = sz;
	}
	uorNumber size(bool isDef = false) {return isDef?m_sizeDef:m_size;}

	bool 	hiden() {return m_hiden;}
	void 	setHiden(const bool& hd) {m_hiden = hd;}
	bool 	fixed() {return m_fixed;}
	void 	setFixed(const bool& fx) {m_fixed = fx;}
	void	clear()	{}
...
class uoHeaderScale	: public uoNumVector<uoRptNumLine>
{
	...
	bool onStoreItems(uoReportLoader* loader = 0, QList<int>* list = NULL, bool erase = false);
...
class uoReportDocFontColl {
	...
	public:
		...
		void clear();
		void copyFrom(uoReportDocFontColl *col);
...
struct uorTextDecor : public uorTextDecorBase {
	public:
		...
		int 				m_alignFlags;		///< Флаг для отрисовки текста (оптимизация) сожержит флаги наличия табуляции и разрыва линий
...
inline uorNumber getMaxRowLength() {return m_maxRowLen;}
#ifdef Q_OS_WIN
	unsigned int 		getWinAligment() const;
#endif
	...
	void				setTextType(uoCellTextType tt, uoReportDoc* doc); //m_textType
	...
	bool 		isIndependentCell() const;
	bool 		skipVisitor();

	QRect 		getCellJoinRect() const;
...
class uoRow : public uoEnumeratedItem, public uoNumVector<uoCell>
{
	public:
		...
		virtual ~uoRow();
	public:
		// тут надо сделать перенумерацию ячеек.
		void setNumber(const int& nm);
		...
		void copyFromTempl(uoRow* fromSrc, uoReportDoc* docThere, uoReportDoc* docSrc, int startColNo = -1, int endColNo = -1);
		...
		void saveItems(uoReportLoader* loader = 0, uoReportSelection* selection = 0, bool erase = false);
...
class uoRowsDoc : public uoNumVector<uoRow>
{
	public:
		...
		void saveItems(uoReportLoader* loader = 0, uoReportSelection* selection = NULL, bool erase = false);
