...
class uoReportDrawHelper
{
	...
	private:
		...
		HRGN m_hrgn;
		bool m_rgnCreated;
        wchar_t *m_drawWCharArr; // http://habrahabr.ru/post/164193/ ( Что такое TCHAR, WCHAR, LPSTR, LPWSTR,LPCTSTR (итд) )
		size_t	m_drawWCharLength;
		bool 	checkWArray(size_t size);
#endif
		QFont* checkFont(uoPainter& painter, uoCell* cell, uoReportDoc* doc);
		uorTextDecorBase m_oldFontProp;
	...
	private:
        void drawTextDH(uoPainter& painter, uorRect& rectCpyCell, QString& text, uoCell* cell);
