	...
	:QObject(parent)
{
	...
	m_strartColRow	= -1;
	uorZeroPointI(m_cellStart);
	uorZeroPointI(m_cellMidle);
	m_currentCell = QPoint(1,1); // Иначе вылет при вставке в новый документ
}
...
void uoReportSelection::accumulateCellSelection()
{
	if (!m_doc)
		return;
...
//	QRect* rect = 0;
	...
	m_rectCellsMidle = m_doc->getCellJoinRect(m_cellStart/*.x(), m_cellStart.y()*/);
	rect2 = m_doc->getCellJoinRect(m_cellMidle/*.x(), m_cellMidle.y()*/);
	if (m_cellMidle.x() > 0)
		m_rectCellsMidle = m_rectCellsMidle.unite(rect2);
	while(rectCM != m_rectCellsMidle){
		rectCM = m_rectCellsMidle;
		for (int row = rectCM.top(); row<=rectCM.bottom(); row++) {
			for (int col = rectCM.left(); col<=rectCM.right(); col++) {
				rect2 = m_doc->getCellJoinRect(QPoint(col,row));
				m_rectCellsMidle = m_rectCellsMidle.unite(rect2);
			}
		}
	}
...
QRect* uoReportSelection::getCellSpanNew()
{
	QRect* rect = NULL;
...
QRect uoReportSelection::getSelectionBound() const
{
	...
	if(m_selMode == uoRst_Document || m_selMode == uoRst_Unknown){
		if (m_selMode == uoRst_Unknown)
			...
			return m_doc->getCellJoinRect(m_currentCell);
		return retVal;
	} else {
		if (!m_selRows->isEmpty()){
