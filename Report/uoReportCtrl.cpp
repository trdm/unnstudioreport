	...
	: QWidget(parent) //, m_cornerWidget(parent)
{
	...
	m_groupListCols = new uoRptGroupItemList;		///< список ректов группировок столбцов
	m_groupListRows = new uoRptGroupItemList;		///< список ректов группировок строк

	m_sectItemListCache = new uoRptSectionItemList;	///< кешь для экземпляров uoRptSectionItem
	m_sectItemListCols = new uoRptSectionItemList;		///< список итемов секций столбцов
	m_sectItemListRows = new uoRptSectionItemList;		///< список итемов секций строк
...
//	m_directDraw 	= true;
	...
	m_fixationType = uorAF_None;
	m_paintLocker = 0;
	setProperty("vievno",QString("vievno: %1").arg(uoReportManager::instance()->nextViewCounter()));
}
...
void uoReportCtrl::setDoc(uoReportDoc* rptDoc)
{
	...
	connect(m_rptDoc, SIGNAL(refresh()), this, SLOT(refreshDoc()));
...
void uoReportCtrl::dropGropItemToCache()
{
	...
	while (!m_groupListCols->isEmpty()) {
		item = m_groupListCols->takeFirst();
		item->clear();
		m_groupListCache->append(item);
	}
	while (!m_groupListRows->isEmpty()) {
		item = m_groupListRows->takeFirst();
		item->clear();
		m_groupListCache->append(item);
	}

	uoRptSectionItem* rsItem = NULL;
	while (!m_sectItemListCols->isEmpty()) {
		rsItem = m_sectItemListCols->takeFirst();
		rsItem->clear();
		m_sectItemListCache->append(rsItem);
	}
	while (!m_sectItemListRows->isEmpty()) {
		rsItem = m_sectItemListRows->takeFirst();
		rsItem->clear();
		m_sectItemListCache->append(rsItem);
	}
}
...
void uoReportCtrl::calcGroupItemPosition(uoRptGroupItem* grItem, uoRptHeaderType rht)
{
	...
	if (rht == uorRhtColumnHeader) {
		...
		if (itemVisible) {
			...
			if (xSize0 > (uorNumber(UORPT_SCALE_FOLD_ITEM_SIZE) + uorNumber(UORPT_OFFSET_LINE)*2)){
		...
		grItem->m_rectMidlePos = yPos + UORPT_SCALE_FOLD_ITEM_SIZE / (uorNumber)2; //		grItem->m_rectMidlePos = yPos + m_charHeightPlus / 2;
			...
			if (sectLevel>0){
				...
				for (i=0; i<spanSectListH->size(); i++){
					...
					if (selectItem)	{
						...
						m_sectItemListRows->append(selectItem);
					}
				}
			...
			if (sectLevel>0){
				...
				for (i=0; i<spanSectListV->size(); i++){
					...
					if (selectItem)	{
						...
						m_sectItemListCols->append(selectItem);
					}
				}
...
//
//	if (!m_groupListRows->isEmpty()) {
//		qDebug() << "m_groupListRows";
//		for (cntr = 0; cntr<m_groupListRows->size(); cntr++) {
//			rgItem = m_groupListRows->at(cntr);
//			qDebug() << rgItem->m_rectIteract << rgItem->m_start << rgItem->m_end;
//		}
//	}
//
//	if (!m_groupListCols->isEmpty()) {
//		qDebug() << "m_groupListCols";
//		for (cntr = 0; cntr<m_groupListCols->size(); cntr++) {
//			rgItem = m_groupListCols->at(cntr);
//			qDebug() << rgItem->m_rectIteract << rgItem->m_start << rgItem->m_end;
//		}
//	}
...
void uoReportCtrl::drawHeaderControlGroup(uoPainter& painter)
{
	...
	if (m_showGroup) {
		...
		if (m_rectGroupCol.height()>noLen)		{
			if (!m_groupListRows->isEmpty()) {
				...
				for (cntr = 0; cntr<m_groupListRows->size(); cntr++) {
					grItem = m_groupListRows->at(cntr);
	...
	if (m_showSection){
		...
		if (!m_sectItemListRows->isEmpty()) {
			painter.setClipRect(m_rectSectionCol); // Устанавливаем область прорисовки. Будем рисовать только в ней.
			for (cntr = 0; cntr<m_sectItemListRows->size(); cntr++) {
				sectItem = m_sectItemListRows->at(cntr);
		...
		if (!m_sectItemListCols->isEmpty()){
			painter.setClipRect(m_rectSectionRow); // Устанавливаем область прорисовки. Будем рисовать только в ней.
			for (cntr = 0; cntr<m_sectItemListCols->size(); cntr++) {
				sectItem = m_sectItemListCols->at(cntr);
				uorRect rect = sectItem->m_rectView;
				painter.setPen(m_drawHelper->m_penText);
				if (sectItem->m_selected){
					painter.fillRect(rect, m_drawHelper->m_brushBlack);
...
//		painter.drawRect(m_rectAll);
	...
	*/
	...
	uorNumber offSet = (uorNumber)2.0;
	...
	if (m_showRuler) {
		...
		*/
		...
		if (m_rectRulerRow.width() > 0) {
			...
			curRct.setLeft(m_rectRulerRow.left()/*+offSet*/);
			curRct.setRight(m_rectRulerRow.right());
			curRct.adjust(1,0,-1,0);

			paintEndTagr = m_rectRulerRow.bottom();
			paintCntTagr = m_rectRulerRow.top() - m_areaMain.shift_RowTop() + 1; //int(1 * m_scaleFactorO);
...
#ifndef uorNumberFloat
				curRct.adjust(uorNumberNull,uorNumberNull,uorNumberNull,-1);
#endif
		...
		if (m_rectRulerCol.width() > 0) {
			...
			curRct.setTop(m_rectRulerCol.top()+offSet);
			curRct.setBottom(m_rectRulerCol.bottom()-offSet);
			
			paintEndTagr = m_rectRulerCol.right();
			paintCntTagr = m_rectRulerCol.left() - m_areaMain.shift_ColLeft() + int(1 * m_scaleFactorO);
...
#ifndef uorNumberFloat
				curRct.adjust(uorNumberNull,uorNumberNull,-1,uorNumberNull);
#endif
				painter.drawRect(curRct);
...
void uoReportCtrl::drawCurentCell(uoPainter& painter)
{
	...
	if (!rect.isEmpty()){
		...
		painter.setPen(m_drawHelper->m_penText);
		uorScaleRectI(rect,m_scaleFactor);
		painter.drawRect(rect);
...
void uoReportCtrl::checkRedrawOption()
{
	...
	&& m_drawToImage == 0)
        ...
        if (m_cashDrawMaVer == m_areaMain.changesVer() &&
                m_cashDrawDocVer == m_rptDoc->getChangeCount() /*&& m_drawToImage == 0*/)	{
...
bool uoReportCtrl::mousePressEventForGroup(QMouseEvent *event, bool isDoubleClick){
	...
	if (m_scaleFactor != 1.0){
		posX = (int)(posX * m_scaleFactorO);
		posY = (int)(posY * m_scaleFactorO);
	}
...
bool uoReportCtrl::mousePressEventForSection(QMouseEvent *event, bool isDoubleClick)
{
	...
	if (m_rectSectionCol.contains(posX, posY) || m_rectSectionRow.contains(posX, posY))
	{
		...
		uoRptSectionItemList* sList = m_sectItemListCols;
		uoRptSectionItemList* sListOther = m_sectItemListRows;
		if (m_rectSectionCol.contains(posX, posY)) {
			rHt = uorRhtColumnHeader;
			sList = m_sectItemListRows;
			sListOther = m_sectItemListCols;
		}
		int i;
...
bool uoReportCtrl::findScaleLocation(uorNumber posX, uorNumber posY, int &scaleNo, uoRptHeaderType rht, bool saveStartRc)
{
    Q_UNUSED(saveStartRc);
	bool retVal = false;
	uoReportDoc* doc =  getDoc();
	if (!doc)
		return retVal;
	...
	uorNumber startPos = uorNumberNull;
	uorNumber scSize = uorNumberNull;

	if (rht == uorRhtRowsHeader){
		cntScale = m_areaMain.firstVisible_RowTop();
		endPos = posY;
		startPos = m_rectDataRegion.top() - m_areaMain.shift_RowTop();
...
bool uoReportCtrl::mousePressEventForRuler(QMouseEvent *event, bool isDoubleClick)
{
	...
	if (findScaleLocation(posX, posY, scaleNo, rhtCur, true)){
		...
		*/
		...
		if (rhtCur == uorRhtColumnHeader){
			...
			if (locType == uoBlt_Left || locType == uoBlt_Right)
			{
				...
				m_curMouseSparesRectStart = m_curMouseSparesRect;
...
/// Обработать перемещение мыши или отпускание.
void uoReportCtrl::mouseEventProcess(QMouseEvent *event, int evType)
{
	bool isMove = (evType == 0) ? true : false;
	bool isRel = (evType == 1) ? true : false;
	//evType = 0 - перемещение
	//evType = 1 - высвобождение
	uorRect rct;
	int line = 0;
		...
		if (m_stateMode == rmsSelectionRule_Left || m_stateMode == rmsSelectionRule_Top){
			if (rmsSelectionRule_Left == m_stateMode) {
				if (findScaleLocation(posX, posY, line,uorRhtRowsHeader)) {
					if (isRel)	{	
						...
						m_selections->rowSelectedMidle(line);
					}
			...
			if (rmsSelectionRule_Top == m_stateMode) {
				if (findScaleLocation(posX, posY, line,uorRhtColumnHeader)) {
					if (isRel)	{	
						...
						m_selections->colSelectedMidle(line);						
					}
			...
			if (distance > QApplication::startDragDistance()) {
				...
				if (m_stateMode == rmsResizeRule_Left) {
					newSize = posY - m_curMouseSparesRectStart.bottom() + m_curMouseSparesRectStart.height();
				} else {
					newSize = posX - m_curMouseSparesRectStart.right() + m_curMouseSparesRectStart.width();
				}
				...
				if (isRel) {
					if (m_stateMode == rmsResizeRule_Top){
						setStateMode(rmsNone); // Надо сбросить иначе пока форматируется документ будем узревать красную линию..
	...
	if (m_stateMode == rmsSelectionCell){
		...
		if (distance > QApplication::startDragDistance() || isCtrlPress) {
			...
			if (!pntCell.isNull()){
				if ((pntCell.x() == m_curentCell.x()) && (pntCell.y() == m_curentCell.y()))	{
				}  else {
					if (isRel) {
...
{
	mouseEventProcess(event, 1);
	uorZeroRectF(m_curMouseSparesRectStart);
}
...
void uoReportCtrl::focusOutEvent ( QFocusEvent * event ){ Q_UNUSED(event);  }
...
bool uoReportCtrl::populateMenu(QMenu* targMenu)
{
	...
	menuSpecial->addAction(m_iteractView->m_actCreateMatrixMini);
...
void uoReportCtrl::onOptionsShow()
{
	...
	dlg->setUsePixCacher(doc->usePictCasher());
	bool oldDirectDraw = m_directDraw;

	int dret = dlg->exec();
	if (dret==1){
		doc->setUsePictCasher(dlg->usePixCacher());
...
void uoReportCtrl::onCreateMatrixMini()
{
	uoReportDoc* doc = getDoc();
	if (!doc)
		return;
	int all_val = 0, row_val = 10, col_val = 10;
	int lenghtAll = 0;
	...
	QString addTextToCell = "Mini Matrix text text text";
	QString cellSrt;
...
void uoReportCtrl::onSetScaleFactor(const qreal sFactor){
	if (sFactor != m_scaleFactor && sFactor != 0) {
...
void uoReportCtrl::onDeleteCells(){
	uoReportDoc* doc = getDoc();
	if (!doc)
		return;
	doc->onDeleteCells(m_selections);
	recalcHeadersRects();
	updateThis();
}
...
void uoReportCtrl::onGridShow(){	if (!m_showGrid) {m_showGrid = true;	recalcHeadersRects();	m_forseRedraw = 1; updateImage(); }}
/// Скрываем/показываем сетку.
void uoReportCtrl::onGridHide(){	if (m_showGrid)	{m_showGrid = false;	recalcHeadersRects();	m_forseRedraw = 1; updateImage(); }}

void uoReportCtrl::onInvisibleCharShow(){	if (!m_drawHelper->m_showInvisiblChar) {		m_drawHelper->m_showInvisiblChar = true;	m_forseRedraw = 1;	updateImage();	}}
void uoReportCtrl::onInvisibleCharHide(){	if (m_drawHelper->m_showInvisiblChar) {		m_drawHelper->m_showInvisiblChar = false;	m_forseRedraw = 1;	updateImage();	}}

void uoReportCtrl::onSheetPanShow(){	if (!m_sPanel->isVisible()) 	{m_sPanel->show();	recalcHeadersRects();	m_forseRedraw = 1; updateImage();}}
void uoReportCtrl::onSheetPanHide(){	if (m_sPanel->isVisible()) 	{m_sPanel->hide();		recalcHeadersRects();	m_forseRedraw = 1; updateImage();}}


/// Скрываем/показываем рамку вокруг окна....
void uoReportCtrl::onFrameShow(){	if (!m_showFrame) 	{m_showFrame = true;	m_forseRedraw = 1;	recalcHeadersRects();	updateImage();}}
void uoReportCtrl::onFrameHide(){	if (m_showFrame) 	{m_showFrame = false;	m_forseRedraw = 1; recalcHeadersRects();	updateImage();}}

/// Скрываем/показываем линейку.
void uoReportCtrl::onRulerShow(){	if (!m_showRuler) 	{m_showRuler = true;	m_forseRedraw = 1;	recalcHeadersRects();	updateImage();}}
void uoReportCtrl::onRulerHide(){	if (m_showRuler) 	{m_showRuler = false;	m_forseRedraw = 1;	recalcHeadersRects();	updateImage();}}

/// Скрываем/показываем секции.
void uoReportCtrl::onSectionShow(){	if (!m_showSection) 	{m_showSection = true;	m_forseRedraw = 1;	recalcHeadersRects();	updateImage();}}
void uoReportCtrl::onSectionHide(){	if (m_showSection) 	{m_showSection = false;	m_forseRedraw = 1;	recalcHeadersRects();	updateImage();}}

/// Скрываем/показываем группировки.
void uoReportCtrl::onGroupShow(){	if (!m_showGroup) 	{m_showGroup = true;	m_forseRedraw = 1;		recalcHeadersRects();	updateImage();}}
void uoReportCtrl::onGroupHide(){	if (m_showGroup) 	{m_showGroup = false;	m_forseRedraw = 1;	recalcHeadersRects();	updateImage();}}
