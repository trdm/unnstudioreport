...
class uoReportDoc : public QObject
{
	...
	public:
		...
		bool flush(uoReportLoader* loader = 0, uoReportSelection* selection = 0, bool erase = false);
		...
		void 				setStoreOptions(QString  filePath, uoRptStoreFormat stFormat = uoRsf_XML); ///< установим опции сохранения
	...
	public:
		...
		uoReportDocFontColl* 	getFontColl() { return m_fontColl; }
...
//		void onSizeChange(uorNumber sizeV, uorNumber sizeH, int row, int col);
	...
	public slots:
		...
		void 	onDeleteCells(uoReportSelection* sel = 0);
...
inline 	void 	setSaveUndoStack(const bool save){ m_saveWithUndoStack = save; }
	...
	protected:
		...
		uorPagePrintSetings* 	m_pagesSetings; /// вспомогательная структура, для управления настройками.
...
//		uoCacheItemizer<uoCellJoin> 		m_cellJoinCash;
	...
	protected:
		...
		uoCellPictCasher* 	 m_pictCasher;
		bool			 	 m_pictCasherUse;
		uoReportDoc*		m_sourceDoc; ///< Исходная таблица-шаблон из которой будет дергаться области для подстановки в результирующий документ.
		QMap<QString,QString> m_varaiableMap;
		QString			m_headerSection;
		QString			m_footerSection;

	public:

inline	bool usePictCasher() const {return m_pictCasherUse;}
		bool setUsePictCasher(bool use) {m_pictCasherUse = use; return true;}
...
inline 	uoReportSelection* 	 selection() {return m_selection; }
		...
		uoCellPictCasher* 	 pictCasher();

		// Исходная таблица. Инициализация и прочее..
		uoReportDoc*	sourceDoc();
		bool	setSourceDocFile(QString fileName);
		bool	setSourceDocXML(QString xml);
		bool	setSourceDoc(uoReportDoc* doc);
		bool	setVariable(QString var, QString value);
		QString variable(QString var);
		// Колонтитулы
		//todoo
		bool	useSectionAsHeader(const QString& name); // верхний
		bool	useSectionAsFooter(const QString& name); // нижний.
		bool	outputSection(const QString& name); ///< Вывести секцию
		bool	joinSection(const QString& name); ///< Присоединить секцию
		bool	getSectionRect(const QString& name, QRect& rect);
		bool	bindValuesToText(QString& name, uoCellTextType tt) const;
		void	updateViews(); /// послать сигнал на перерисовку
